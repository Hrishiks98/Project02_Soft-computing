clear;
clc;
b=readtable('Data_set_4.csv'); %reading from dataset
a=(table2array(b));
A=a(:,2:size(a,2));
...
%training
b=ones(size(input_norm_tr,1),1); %bias
...
% testing
 ...
 Earr=(MSE_output/(size(test_set,1)*k))
...
xlabel('Sample no.');
ylabel('Active No. of Cases');
title('ACTIVE CASES PREDICTION');
...
legend('EXACT','PREDICTED');
xlabel('Sample no.');
ylabel('SERIOUS/CRITICAL CASES');
title('SERIOUS/CRITICAL PREDICTION');
